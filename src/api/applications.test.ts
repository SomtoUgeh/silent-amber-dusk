...
describe("fetchApplications", () => {
  ...
  describe("basic functionality", () => {
    it("should fetch applications with pagination params", async () => {
      const mockApplications: Application[] = [createMockApplication()];
  ...
  describe("pagination link header parsing", () => {
    it.each([
      ...
      "should parse Link header correctly when next page exists $description",
      async ({ linkHeader }) => {
        const mockResponse = {
          ok: true,
          json: vi.fn().mockResolvedValue([]),
    ...
    it.each([
      ...
      "should return hasNextPage as false $description",
      async ({ linkHeader, page }) => {
