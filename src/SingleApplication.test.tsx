...
describe("SingleApplication", () => {
  describe("rendering application data", () => {
    ...
    it("should handle long company names", () => {
      ...
      expect(
        screen.getByText("Expeditors International of Washington Inc."),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("currency formatting", () => {
    it.each([
      { amount: 10300, expected: "£10,300", description: "small amounts" },
      {
        amount: 100500,
        expected: "£100,500",
        description: "large amounts with commas",
      },
  ...
  describe("date formatting", () => {
    it.each([
      ...
      "should format dates as DD-MM-YYYY for $description",
      ({ date_created, expiry_date, expectedCreated, expectedExpiry }) => {
        const application = createMockApplication({
          date_created,
          expiry_date,
        });
  ...
  describe("email interaction", () => {
    it("should make email address clickable", () => {
      ...
      const emailLink = screen.getByRole("link", {
        name: /Email John Doe at test@example\.com/i,
      });
